#!/bin/bash
# One-time VPS setup for GitHub Actions deploy (family-recipes).
...
echo "==> Sudoers"
sudo tee /etc/sudoers.d/family-recipes-deploy > /dev/null <<SUDOERS
deploy ALL=(ALL) NOPASSWD: /bin/rm -rf /var/www/family-recipes/*
deploy ALL=(ALL) NOPASSWD: /bin/cp -r * /var/www/family-recipes/
deploy ALL=(ALL) NOPASSWD: /usr/bin/chown -R caddy\:deploy /var/www/family-recipes
...
if [ ! -f /home/deploy/.ssh/id_ed25519 ]; then
  sudo -u deploy ssh-keygen -t ed25519 -C "github-actions-family-recipes" -f /home/deploy/.ssh/id_ed25519 -N ""
fi
if [ ! -f /home/deploy/.ssh/id_ed25519_github ]; then
  sudo -u deploy ssh-keygen -t ed25519 -C "deploy-git-family-recipes" -f /home/deploy/.ssh/id_ed25519_github -N ""
fi
...
echo ""
echo "Done. Add to GitHub repo jm-sky/family-recipes:"
